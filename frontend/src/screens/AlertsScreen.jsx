...
export default function AlertsScreen() {
  ...
  React.useEffect(() => {
    const derivedAlerts = [];
    if (state.farms && Array.isArray(state.farms)) {
      state.farms.forEach(f => {
        const score = f.analysis?.satellite?.farm_health_score || 0;
        if (f.analysis && score <= 50) {
          const isCritical = score <= 25;
          derivedAlerts.push({
            id: `alert-${f.id}`,
            category: 'Active',
            unread: true,
            field: f.farm_name || 'Data unavailable',
            time: new Date().toLocaleDateString(),
            title: f.analysis.intervention?.action || 'Health Alert',
            body: f.analysis.intervention?.reasoning || 'Farm health score is below optimal thresholds.',
            icon: Bell,
            bg: isCritical ? '#FEE2E2' : '#FEF3C7',
            iconColor: isCritical ? '#DC2626' : '#D97706',
            details: f.analysis.risk?.llm_explanation || 'Data unavailable',
            recommendation: f.analysis.intervention?.action || 'Data unavailable'
          });
        }
      });
    }
